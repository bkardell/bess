...
define([],function(){
    ...
    var _old = PageBus.publish;
    PageBus.publish = function(){
        var args = arguments;
        setTimeout(function(){
            _old.apply(PageBus,args);
        },1);
    }

    var makePublisher = function(level){
        return function(str){
            PageBus.publish("logger." + level,{body: str});
        }
    }

    window.Logger = {
        trace: makePublisher("trace"),
        debug: makePublisher("debug"),
        info: makePublisher("info"),
        warn: makePublisher("warn"),
        error: makePublisher("error"),
        fatal: makePublisher("fatal")
    }
